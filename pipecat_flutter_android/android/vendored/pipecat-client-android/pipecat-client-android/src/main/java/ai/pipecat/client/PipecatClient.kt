        ...
        override fun onMessage(msg: MsgServerToClient) = thread.runOnThread {
            ...
            try {
                when (msg.type) {
                    ...
                    MsgServerToClient.Type.LlmFunctionCallStarted -> {
                        val data = JSON_INSTANCE.decodeFromJsonElement<LLMFunctionCallStartedData>(msg.data)
                        callbacks.onLLMFunctionCallStarted(data)
                    }

                    MsgServerToClient.Type.LlmFunctionCallInProgress -> {
                        val data = JSON_INSTANCE.decodeFromJsonElement<LLMFunctionCallInProgressData>(msg.data)
                        callbacks.onLLMFunctionCallInProgress(data)
                    }

                    MsgServerToClient.Type.LlmFunctionCallStopped -> {
                        val data = JSON_INSTANCE.decodeFromJsonElement<LLMFunctionCallStoppedData>(msg.data)
                        callbacks.onLLMFunctionCallStopped(data)
                    }
