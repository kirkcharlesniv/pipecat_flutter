...
public protocol PipecatClientDelegate: AnyObject {
    ...
    /// Invoked when the LLM attempts to invoke a function via the deprecated
    /// `llm-function-call` message. Modern pipecat servers (0.0.102+) emit the
    /// three lifecycle messages below instead; see `onLLMFunctionCallStarted`,
    /// `onLLMFunctionCallInProgress`, and `onLLMFunctionCallStopped`. This
    /// callback is kept for back-compat with servers still emitting the old name.
    func onLLMFunctionCall(functionCallData: LLMFunctionCallData, onResult: ((Value) async -> Void)) async

    /// Invoked when the LLM has decided to call a function (no args yet).
    /// Modern pipecat lifecycle message: `llm-function-call-started`.
    func onLLMFunctionCallStarted(data: LLMFunctionCallStartedData)

    /// Invoked when the LLM function call is in flight with full call data.
    /// Modern pipecat lifecycle message: `llm-function-call-in-progress`.
    /// The provided callback must be provided with a return value.
    func onLLMFunctionCallInProgress(data: LLMFunctionCallInProgressData, onResult: ((Value) async -> Void)) async

    /// Invoked when the LLM function call has finished or been cancelled.
    /// Modern pipecat lifecycle message: `llm-function-call-stopped`.
    func onLLMFunctionCallStopped(data: LLMFunctionCallStoppedData)
...
extension PipecatClientDelegate {
    ...
    public func onLLMFunctionCallInProgress(data: LLMFunctionCallInProgressData, onResult: ((Value) async -> Void)) async {}
